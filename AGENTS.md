...
3. The plugin never writes harness settings (the `startup.quiet` hint is advisory text only).
...
a dim quiet-advisory line is embedded in the widget render.
`before_agent_start`: one-shot dismissal when `cfg.dismiss`.

## Key Directories

- `src/` — all runtime code (4 modules above).
- `tests/` — 4 suites mirroring module names + shared `tests/helpers.ts`.
- `scripts/smoke.ts` — host-free assertion pass (35 checks).
...
npm test            # node --test tests/*.test.ts — expect 85 passing
npm run smoke       # node scripts/smoke.ts — expect 35 "ok" lines, exit 0
```
...
- Suite map: `config.test.ts` (22 — inert rule, layers, coercion, tokens),
  `dashboard.test.ts` (28 — parity, delta rendering, geometry sweep),
  `host.test.ts` (17 — probe/fetch classification), `lifecycle.test.ts`
  (18 — omp vs pi routing through mock hosts). Total 85.
